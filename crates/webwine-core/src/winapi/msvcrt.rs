...
pub fn register(r: &mut WinApiRegistry) {
    let fns: &[(&str, &str, super::HandlerFn)] = &[
        ...
        ("msvcrt.dll",    "__p__environ",          |c| { let v = 0x7FFD_F300u32; c.ret_cdecl(v); Handled::Ok }),
        ("msvcrt.dll",    "_get_initial_narrow_environment", |c| { c.ret_cdecl(0); Handled::Ok }),
        ("msvcrt.dll",    "_get_narrow_winmain_command_line", |c| { c.ret_cdecl(0); Handled::Ok }),
        ...
        ("msvcrt.dll",    "__current_exception_context", stub_zero_cdecl_0),
        ...
        ("vcruntime140.dll", "__C_specific_handler", stub_zero_cdecl_1),
    ];
...
fn memcmp(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_cdecl(r as u32); Handled::Ok
}

fn strlen(ctx: &mut ApiContext) -> Handled {
    let p = ctx.arg(0);
    let s = ctx.cstr(p);
    ctx.ret_cdecl(s.len() as u32); Handled::Ok
}
...
fn strstr(ctx: &mut ApiContext) -> Handled {
    ...
    let r = hay.find(&needle[..]).map(|i| ctx.arg(0) + i as u32).unwrap_or(0);
    ctx.ret_cdecl(r); Handled::Ok
}
...
fn putchar(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_cdecl(c as u32); Handled::Ok
}
...
fn printf(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_cdecl(n as u32); Handled::Ok
}
...
fn stdio_vfprintf(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.console.stdout.extend_from_slice(result.as_bytes());
    ctx.ret_cdecl(result.len() as u32); Handled::Ok
}
...
fn fwrite(ctx: &mut ApiContext) -> Handled {
    ...
    let size = ctx.arg(1);
    let n    = ctx.arg(2);
    let bytes = ctx.memory.read_bytes(buf, (size * n) as usize).unwrap_or_default();
    ctx.console.stdout.extend_from_slice(&bytes);
    ctx.ret_cdecl(n); Handled::Ok
}
...
fn format_string(ctx: &ApiContext, fmt: &str, first_arg: u32) -> String {
    ...
    while i < chars.len() {
        ...
        match spec {
            'd' | 'i' => {
                let v = ctx.memory.read_u32(ctx.cpu.esp + 4 + 4 * arg_idx).unwrap_or(0) as i32;
                out.push_str(&v.to_string()); arg_idx += 1;
            }
            'u' => {
                let v = ctx.memory.read_u32(ctx.cpu.esp + 4 + 4 * arg_idx).unwrap_or(0);
                out.push_str(&v.to_string()); arg_idx += 1;
            }
            'x' => {
                let v = ctx.memory.read_u32(ctx.cpu.esp + 4 + 4 * arg_idx).unwrap_or(0);
                out.push_str(&format!("{v:x}")); arg_idx += 1;
            }
            'X' => {
                let v = ctx.memory.read_u32(ctx.cpu.esp + 4 + 4 * arg_idx).unwrap_or(0);
                out.push_str(&format!("{v:X}")); arg_idx += 1;
            ...
            'p' => {
                let v = ctx.memory.read_u32(ctx.cpu.esp + 4 + 4 * arg_idx).unwrap_or(0);
                out.push_str(&format!("{v:08X}")); arg_idx += 1;
            }
            'c' => {
                let v = ctx.memory.read_u32(ctx.cpu.esp + 4 + 4 * arg_idx).unwrap_or(0) as u8;
                out.push(v as char); arg_idx += 1;
            }
            's' => {
                let ptr = ctx.memory.read_u32(ctx.cpu.esp + 4 + 4 * arg_idx).unwrap_or(0);
                out.push_str(&ctx.memory.read_cstr(ptr)); arg_idx += 1;
            }
            '%' => out.push('%'),
            'n' => { arg_idx += 1; } // skip
...
fn stub_zero_cdecl_0(c: &mut ApiContext) -> Handled { c.ret_cdecl(0); Handled::Ok }
fn stub_zero_cdecl_1(c: &mut ApiContext) -> Handled { c.ret_cdecl(0); Handled::Ok }
fn stub_zero_cdecl_2(c: &mut ApiContext) -> Handled { c.ret_cdecl(0); Handled::Ok }
fn stub_zero_cdecl_3(c: &mut ApiContext) -> Handled { c.ret_cdecl(0); Handled::Ok }
fn stub_void_1_cdecl(c: &mut ApiContext) -> Handled { c.ret_cdecl(0); Handled::Ok }
fn stub_void_0(c: &mut ApiContext)       -> Handled { c.ret_cdecl(0); Handled::Ok }
fn stub_void_1(c: &mut ApiContext)       -> Handled { c.ret_cdecl(0); Handled::Ok }
