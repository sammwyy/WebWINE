use crate::vm::handles::{
    CURRENT_PROCESS, CURRENT_THREAD, INVALID_HANDLE,
...
pub fn register(r: &mut WinApiRegistry) {
    let fns: &[(&str, &str, super::HandlerFn)] = &[
        ...
        ("kernel32.dll", "GetCurrentProcessId",           get_current_process_id),
        ...
        ("kernel32.dll", "QueryPerformanceCounter",       query_perf_counter),
        ...
        ("kernel32.dll", "InitializeCriticalSectionAndSpinCount", stub_true_2),
        ...
        ("kernel32.dll", "GetConsoleCP",                  |c| { c.ret_stdcall(437, 0); Handled::Ok }),
        ("kernel32.dll", "GetConsoleOutputCP",            |c| { c.ret_stdcall(437, 0); Handled::Ok }),
        ...
        ("kernel32.dll", "GetACP",                        |c| { c.ret_stdcall(1252, 0); Handled::Ok }),
        ("kernel32.dll", "GetOEMCP",                      |c| { c.ret_stdcall(437, 0); Handled::Ok }),
        ("kernel32.dll", "IsValidCodePage",               stub_true_1),
        ("kernel32.dll", "GetCPInfo",                     stub_false_2),
        ("kernel32.dll", "LCMapStringW",                  stub_zero_1),
        ("kernel32.dll", "FindFirstFileA",                |c| { c.ret_stdcall(INVALID_HANDLE, 2); Handled::Ok }),
        ...
        ("kernel32.dll", "EncodePointer",                 |c| { let p = c.arg(0); c.ret_stdcall(p ^ 0xDEAD, 1); Handled::Ok }),
        ("kernel32.dll", "DecodePointer",                 |c| { let p = c.arg(0); c.ret_stdcall(p ^ 0xDEAD, 1); Handled::Ok }),
        ("kernel32.dll", "Sleep",                         stub_void_1),
        ("kernel32.dll", "WaitForSingleObject",           |c| { c.ret_stdcall(0, 2); Handled::Ok }), // WAIT_OBJECT_0
        ...
        ("kernel32.dll", "InterlockedCompareExchange",    interlocked_cmpxchg),
    ];
...
fn get_std_handle(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_stdcall(h, 1); Handled::Ok
}
...
fn write_file(ctx: &mut ApiContext) -> Handled {
    ...
    let count  = ctx.arg(2);
    let out    = ctx.arg(3);
    let bytes  = ctx.memory.read_bytes(buf, count as usize).unwrap_or_default();
    route_output(handle, &bytes, ctx);
    if out != 0 { let _ = ctx.memory.write_u32(out, bytes.len() as u32); }
    ctx.ret_stdcall(1, 5); Handled::Ok
}
...
fn write_console_w(ctx: &mut ApiContext) -> Handled {
    ...
    if out != 0 { let _ = ctx.memory.write_u32(out, count); }
    ctx.ret_stdcall(1, 5); Handled::Ok
}
...
fn read_file(ctx: &mut ApiContext) -> Handled {
    ...
    if out != 0 { let _ = ctx.memory.write_u32(out, n as u32); }
    ctx.ret_stdcall(if n > 0 { 1 } else { 0 }, 5); Handled::Ok
}

fn close_handle(ctx: &mut ApiContext) -> Handled {
    let h = ctx.arg(0);
    ctx.handles.remove(h);
    ctx.ret_stdcall(1, 1); Handled::Ok
}

fn get_last_error(ctx: &mut ApiContext) -> Handled {
    let e = ctx.cpu.last_error;
    ctx.ret_stdcall(e, 0); Handled::Ok
}
...
fn get_process_heap(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(0x1000_0000, 0); Handled::Ok
}
...
fn heap_create(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(0x1000_0000, 3); Handled::Ok
}

fn virtual_alloc(ctx: &mut ApiContext) -> Handled {
    let addr = ctx.arg(0);
    let size = ctx.arg(1);
    let ptr = if addr != 0 { addr } else { ctx.heap_alloc(size) };
    ctx.ret_stdcall(ptr, 4); Handled::Ok
}

fn virtual_protect(ctx: &mut ApiContext) -> Handled {
    let old_out = ctx.arg(3);
    if old_out != 0 { let _ = ctx.memory.write_u32(old_out, 0x40); }
    ctx.ret_stdcall(1, 4); Handled::Ok
}

fn get_module_handle_a(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(ctx.memory.regions.first().map(|r| r.base).unwrap_or(0x0040_0000), 1);
    Handled::Ok
}
...
fn get_module_filename_a(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_stdcall(n as u32, 3); Handled::Ok
}
...
fn get_command_line_a(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_stdcall(va, 0); Handled::Ok
}
...
fn get_startup_info_a(ctx: &mut ApiContext) -> Handled {
    let p = ctx.arg(0);
    if p != 0 { let _ = ctx.memory.write_bytes(p, &[0u8; 68]); }
    ctx.ret_stdcall(0, 1); Handled::Ok
}

fn get_current_process_id(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(ctx.pid, 0); Handled::Ok
}

fn get_current_thread_id(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(ctx.pid * 100, 0); Handled::Ok
}

fn get_current_process(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(CURRENT_PROCESS, 0); Handled::Ok
}

fn get_current_thread(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(CURRENT_THREAD, 0); Handled::Ok
}

fn get_system_time(ctx: &mut ApiContext) -> Handled {
    let p = ctx.arg(0);
    if p != 0 { let _ = ctx.memory.write_bytes(p, &[0u8; 8]); }
    ctx.ret_stdcall(0, 1); Handled::Ok
}
...
fn query_perf_freq(ctx: &mut ApiContext) -> Handled {
    let p = ctx.arg(0);
    if p != 0 { let _ = ctx.memory.write_bytes(p, &[0x40, 0x42, 0x0F, 0, 0, 0, 0, 0]); }
    ctx.ret_stdcall(1, 1); Handled::Ok
}

fn get_file_type(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(2, 1); Handled::Ok // FILE_TYPE_CHAR
}

fn dup_handle(ctx: &mut ApiContext) -> Handled {
    let out = ctx.arg(5);
    if out != 0 { let _ = ctx.memory.write_u32(out, ctx.arg(2)); }
    ctx.ret_stdcall(1, 7); Handled::Ok
}
...
fn tls_alloc(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(0, 0); Handled::Ok
}

fn tls_set(ctx: &mut ApiContext) -> Handled {
    ctx.ret_stdcall(1, 2); Handled::Ok
}
...
fn multibyte_to_widechar(ctx: &mut ApiContext) -> Handled {
    ...
    let dst  = ctx.arg(4);
    let dstl = ctx.arg(5);
    let s = if srcl == 0xFFFF_FFFF { ctx.cstr(src) }
            else { String::from_utf8_lossy(&ctx.memory.read_bytes(src, srcl as usize).unwrap_or_default()).into_owned() };
    let wide: Vec<u16> = s.encode_utf16().collect();
    let out_len = if dstl == 0 { wide.len() + 1 } else {
        if dst != 0 {
            for (i, &c) in wide.iter().take(dstl as usize).enumerate() {
                let _ = ctx.memory.write_u16(dst + (i as u32)*2, c);
            }
            if (wide.len() as u32) < dstl { let _ = ctx.memory.write_u16(dst + wide.len() as u32 * 2, 0); }
        }
        wide.len().min(dstl as usize) + 1
    };
    ctx.ret_stdcall(out_len as u32, 6); Handled::Ok
}
...
fn widechar_to_multibyte(ctx: &mut ApiContext) -> Handled {
    ...
    let dst  = ctx.arg(4);
    let dstl = ctx.arg(5);
    let nchars = if srcl == 0xFFFF_FFFF { ctx.memory.read_wstr(src).len() + 1 } else { srcl as usize };
    let s = ctx.memory.read_wstr(src);
    let bytes = s.as_bytes();
    let n = bytes.len().min(if dstl == 0 { 0 } else { dstl as usize });
    if n > 0 && dst != 0 { let _ = ctx.memory.write_bytes(dst, &bytes[..n]); }
    ctx.ret_stdcall(if dstl == 0 { nchars as u32 } else { n as u32 }, 8); Handled::Ok
}
...
fn interlocked_inc(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_stdcall(v, 1); Handled::Ok
}
...
fn interlocked_xchg(ctx: &mut ApiContext) -> Handled {
    ...
    ctx.ret_stdcall(old, 2); Handled::Ok
}
...
fn interlocked_cmpxchg(ctx: &mut ApiContext) -> Handled {
    ...
    let cmp = ctx.arg(2);
    let old = ctx.memory.read_u32(p).unwrap_or(0);
    if old == cmp { let _ = ctx.memory.write_u32(p, new); }
    ctx.ret_stdcall(old, 3); Handled::Ok
}

// ── stub helpers ─────────────────────────────────────────────────────────────

fn stub_zero_0 (c: &mut ApiContext) -> Handled { c.ret_stdcall(0, 0); Handled::Ok }
fn stub_zero_1 (c: &mut ApiContext) -> Handled { c.ret_stdcall(0, 1); Handled::Ok }
fn stub_true_1 (c: &mut ApiContext) -> Handled { c.ret_stdcall(1, 1); Handled::Ok }
fn stub_true_2 (c: &mut ApiContext) -> Handled { c.ret_stdcall(1, 2); Handled::Ok }
fn stub_false_1(c: &mut ApiContext) -> Handled { c.ret_stdcall(0, 1); Handled::Ok }
fn stub_false_2(c: &mut ApiContext) -> Handled { c.ret_stdcall(0, 2); Handled::Ok }
fn stub_void_1 (c: &mut ApiContext) -> Handled { c.ret_stdcall(0, 1); Handled::Ok }
fn stub_void_2 (c: &mut ApiContext) -> Handled { c.ret_stdcall(0, 2); Handled::Ok }
fn stub_invalid_handle(c: &mut ApiContext) -> Handled { c.ret_stdcall(INVALID_HANDLE, 7); Handled::Ok }
