...
pub fn run_slice(
    ...
    loop {
        ...
        if proc.cpu.eip >= TRAMPOLINE_BASE {
            ...
            match api.dispatch(va, &mut ctx) {
                ...
                Some(Handled::Unimplemented) | None => {
                    let name = api.lookup_name(va)
                        .map(|(d, n)| format!("{d}!{n}"))
                        .unwrap_or_else(|| format!("0x{va:08X}"));
                    logs.log(LogLevel::Warn, "api",
                        &format!("[api] unimplemented: {name} — returning 0"), Some(proc.pid));
        ...
        match step(proc) {
            ...
            StepResult::ApiTrap(va) => { proc.cpu.eip = va; /* handled next iteration */ }
            StepResult::Exit(code)  => {
                proc.state = ProcessState::Exited { exit_code: code };
                break;
            }
            StepResult::Fault(r) => {
                logs.log(LogLevel::Error, "cpu",
                    &format!("[cpu] fault at EIP=0x{:08X}: {r}", proc.cpu.eip), Some(proc.pid));
...
pub fn step(proc: &mut GuestProcess) -> StepResult {
    ...
    if instr.is_invalid() {
        return StepResult::Fault(format!("invalid opcode at 0x{eip:08X}: {:02X?}", &bytes[..bytes.len().min(4)]));
    }
...
fn execute(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    ...
    match instr.mnemonic() {
        ...
        Cdq  => { cpu.edx = if cpu.eax >> 31 != 0 { 0xFFFF_FFFF } else { 0 }; StepResult::Continue }
        Cwde => { cpu.eax = (cpu.eax as i16 as i32) as u32; StepResult::Continue }
        ...
        Jle   => exec_jcc(instr, cpu, get_zf(cpu.eflags) || get_sf(cpu.eflags) != get_of(cpu.eflags)),
        Jg    => exec_jcc(instr, cpu, !get_zf(cpu.eflags) && get_sf(cpu.eflags) == get_of(cpu.eflags)),
        ...
        Loop  => { cpu.ecx = cpu.ecx.wrapping_sub(1); exec_jcc(instr, cpu, cpu.ecx != 0) }
        Loope => { cpu.ecx = cpu.ecx.wrapping_sub(1); exec_jcc(instr, cpu, cpu.ecx != 0 && get_zf(cpu.eflags)) }
        Loopne=> { cpu.ecx = cpu.ecx.wrapping_sub(1); exec_jcc(instr, cpu, cpu.ecx != 0 && !get_zf(cpu.eflags)) }
        ...
        Setle => exec_setcc(instr, cpu, mem, get_zf(cpu.eflags) || get_sf(cpu.eflags) != get_of(cpu.eflags)),
        Setg  => exec_setcc(instr, cpu, mem, !get_zf(cpu.eflags) && get_sf(cpu.eflags) == get_of(cpu.eflags)),
        ...
        Leave => {
            ...
            match mem.read_u32(cpu.esp) {
                Ok(v) => { cpu.ebp = v; cpu.esp = cpu.esp.wrapping_add(4); StepResult::Continue }
        ...
        Movsd => exec_movs(instr, cpu, mem, 4),
        ...
        Lodsd => { match mem.read_u32(cpu.esi) { Ok(v) => { cpu.eax = v; cpu.esi = cpu.esi.wrapping_add(4); StepResult::Continue } Err(e) => StepResult::Fault(e.to_string()) } }
        Lodsb => { match mem.read_u8(cpu.esi)  { Ok(v) => { cpu.eax = (cpu.eax & 0xFFFFFF00) | v as u32; cpu.esi = cpu.esi.wrapping_add(1); StepResult::Continue } Err(e) => StepResult::Fault(e.to_string()) } }

        Int3  => StepResult::Fault("INT3 breakpoint".into()),
        Int   => StepResult::Fault(format!("INT 0x{:02X}", instr.immediate8())),
        Hlt   => StepResult::Exit(0),

        other => StepResult::Fault(format!("unimplemented: {:?} at 0x{:08X}", other, cpu.eip.wrapping_sub(instr.len() as u32))),
    }
}
...
pub fn read_reg(r: Register, cpu: &X86Cpu) -> u32 {
    match r {
        Register::EAX => cpu.eax, Register::ECX => cpu.ecx,
        Register::EDX => cpu.edx, Register::EBX => cpu.ebx,
        Register::ESP => cpu.esp, Register::EBP => cpu.ebp,
        Register::ESI => cpu.esi, Register::EDI => cpu.edi,
        Register::AX  => cpu.eax & 0xFFFF, Register::CX  => cpu.ecx & 0xFFFF,
        Register::DX  => cpu.edx & 0xFFFF, Register::BX  => cpu.ebx & 0xFFFF,
        Register::SP  => cpu.esp & 0xFFFF, Register::BP  => cpu.ebp & 0xFFFF,
        Register::SI  => cpu.esi & 0xFFFF, Register::DI  => cpu.edi & 0xFFFF,
        Register::AL  => cpu.eax & 0xFF,   Register::CL  => cpu.ecx & 0xFF,
        Register::DL  => cpu.edx & 0xFF,   Register::BL  => cpu.ebx & 0xFF,
        Register::AH  => (cpu.eax >> 8) & 0xFF, Register::CH => (cpu.ecx >> 8) & 0xFF,
        Register::DH  => (cpu.edx >> 8) & 0xFF, Register::BH => (cpu.ebx >> 8) & 0xFF,
...
pub fn write_reg(r: Register, v: u32, cpu: &mut X86Cpu) {
    match r {
        Register::EAX => cpu.eax = v, Register::ECX => cpu.ecx = v,
        Register::EDX => cpu.edx = v, Register::EBX => cpu.ebx = v,
        Register::ESP => cpu.esp = v, Register::EBP => cpu.ebp = v,
        Register::ESI => cpu.esi = v, Register::EDI => cpu.edi = v,
...
fn reg_size(r: Register) -> usize {
    match r {
        ...
        Register::AH | Register::CH | Register::DH | Register::BH => 1,
        Register::AX | Register::CX | Register::DX | Register::BX |
        Register::SP | Register::BP | Register::SI | Register::DI  => 2,
        _ => 4,
    }
}
...
fn calc_addr(instr: &Instruction, cpu: &X86Cpu) -> u32 {
    ...
    base.wrapping_add(index.wrapping_mul(scale)).wrapping_add(disp)
}

// Read operand value (op_idx = 0 for dst, 1 for src usually)
fn read_op(instr: &Instruction, i: u32, cpu: &X86Cpu, mem: &GuestMemory) -> std::result::Result<u32, String> {
...
fn write_op(instr: &Instruction, i: u32, val: u32, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> std::result::Result<(), String> {
    match instr.op_kind(i) {
        OpKind::Register => { write_reg(instr.op_register(i), val, cpu); Ok(()) }
        OpKind::Memory => {
            let addr = calc_addr(instr, cpu);
            (match mem_size(instr) {
...
fn fault(e: impl std::fmt::Display) -> StepResult { StepResult::Fault(e.to_string()) }
...
fn exec_mov(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    ...
    if matches!(dst_reg, Register::CS|Register::DS|Register::ES|Register::FS|Register::GS|Register::SS) {
...
fn exec_movzx(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    match read_op(instr, 1, cpu, mem) {
        Err(e) => fault(e),
        Ok(v) => {
            let masked = match op_size(instr, 1) { 1 => v & 0xFF, 2 => v & 0xFFFF, _ => v };
            match write_op(instr, 0, masked, cpu, mem) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
        }
    }
}
...
fn exec_movsx(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    match read_op(instr, 1, cpu, mem) {
        Err(e) => fault(e),
        Ok(v) => {
            let extended = match op_size(instr, 1) { 1 => (v as i8 as i32) as u32, 2 => (v as i16 as i32) as u32, _ => v };
            match write_op(instr, 0, extended, cpu, mem) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
        }
    }
}

fn exec_xchg(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let a = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
    let b = match read_op(instr, 1, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
    if let Err(e) = write_op(instr, 0, b, cpu, mem) { return fault(e); }
    if let Err(e) = write_op(instr, 1, a, cpu, mem) { return fault(e); }
    StepResult::Continue
}
...
fn exec_push(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let v = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
    cpu.esp = cpu.esp.wrapping_sub(4);
    match mem.write_u32(cpu.esp, v) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
}

fn exec_pop(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let v = match mem.read_u32(cpu.esp) { Err(e) => return fault(e), Ok(v) => v };
    cpu.esp = cpu.esp.wrapping_add(4);
    match write_op(instr, 0, v, cpu, mem) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
}

fn exec_pushad(cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let saved_esp = cpu.esp;
    for &v in &[cpu.eax, cpu.ecx, cpu.edx, cpu.ebx, saved_esp, cpu.ebp, cpu.esi, cpu.edi] {
        cpu.esp = cpu.esp.wrapping_sub(4);
        if let Err(e) = mem.write_u32(cpu.esp, v) { return fault(e); }
    }
    StepResult::Continue
}
...
fn exec_popad(cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    ...
    for v in vals.iter_mut() {
        *v = match mem.read_u32(cpu.esp) { Err(e) => return fault(e), Ok(v) => v };
        cpu.esp = cpu.esp.wrapping_add(4);
    }
    cpu.edi = vals[0]; cpu.esi = vals[1]; cpu.ebp = vals[2];
    // vals[3] = esp (ignored)
    cpu.ebx = vals[4]; cpu.edx = vals[5]; cpu.ecx = vals[6]; cpu.eax = vals[7];
    StepResult::Continue
}
...
fn exec_pushfd(cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let f = cpu.eflags;
    cpu.esp = cpu.esp.wrapping_sub(4);
    match mem.write_u32(cpu.esp, f) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
}

fn exec_popfd(cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    match mem.read_u32(cpu.esp) {
        Err(e) => fault(e),
        Ok(v) => { cpu.eflags = v; cpu.esp = cpu.esp.wrapping_add(4); StepResult::Continue }
    }
}

enum AluOp { Add, Sub, Adc, Sbb, And, Or, Xor, Cmp, Test }
...
fn exec_alu(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory, op: AluOp) -> StepResult {
    ...
    let (result, write_back) = match op {
        AluOp::Add => { let r = dst.wrapping_add(src); if sz==1 { set_add8(&mut cpu.eflags, dst as u8, src as u8, r as u8) } else { set_add32(&mut cpu.eflags, dst, src, r) }; (r, true) }
        AluOp::Adc => { let c = get_cf(cpu.eflags) as u32; let r = dst.wrapping_add(src).wrapping_add(c); set_add32(&mut cpu.eflags, dst, src, r); (r, true) }
        AluOp::Sub | AluOp::Cmp => { let r = dst.wrapping_sub(src); if sz==1 { set_sub8(&mut cpu.eflags, dst as u8, src as u8, r as u8) } else { set_sub32(&mut cpu.eflags, dst, src, r) }; (r, matches!(op, AluOp::Sub)) }
        AluOp::Sbb => { let c = get_cf(cpu.eflags) as u32; let r = dst.wrapping_sub(src).wrapping_sub(c); set_sub32(&mut cpu.eflags, dst, src, r); (r, true) }
        AluOp::And | AluOp::Test => { let r = dst & src; set_szp(&mut cpu.eflags, r); cpu.eflags &= !(CF | OF); (r, matches!(op, AluOp::And)) }
        AluOp::Or  => { let r = dst | src; set_szp(&mut cpu.eflags, r); cpu.eflags &= !(CF | OF); (r, true) }
        AluOp::Xor => { let r = dst ^ src; set_szp(&mut cpu.eflags, r); cpu.eflags &= !(CF | OF); (r, true) }
    };

    if write_back {
        match write_op(instr, 0, result, cpu, mem) { Err(e) => return fault(e), Ok(()) => {} }
    }
    StepResult::Continue
}
...
fn exec_neg(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    ...
    match write_op(instr, 0, r, cpu, mem) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
}
...
fn exec_imul(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    match instr.op_count() {
        1 => {
            let src = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
            let r = (cpu.eax as i32 as i64) * (src as i32 as i64);
            cpu.eax = r as u32; cpu.edx = (r >> 32) as u32;
            let overflow = cpu.edx != if cpu.eax >> 31 != 0 { 0xFFFF_FFFF } else { 0 };
            if overflow { cpu.eflags |= CF | OF; } else { cpu.eflags &= !(CF | OF); }
        }
        2 => {
            let dst = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
            let src = match read_op(instr, 1, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
            let r = ((dst as i32 as i64) * (src as i32 as i64)) as u32;
            if let Err(e) = write_op(instr, 0, r, cpu, mem) { return fault(e); }
        }
        _ => {
            let src1 = match read_op(instr, 1, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
            let src2 = match read_op(instr, 2, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
...
fn exec_idiv(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let divisor = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v } as i32;
    if divisor == 0 { return StepResult::Fault("division by zero".into()); }
...
fn exec_div(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let divisor = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v } as u64;
    if divisor == 0 { return StepResult::Fault("division by zero".into()); }
...
fn exec_shift(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory, op: ShiftOp) -> StepResult {
    let dst = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
    let cnt = (match read_op(instr, 1, cpu, mem) { Err(e) => return fault(e), Ok(v) => v } & 0x1F) as u32;
    if cnt == 0 { return StepResult::Continue; }
    let result = match op {
        ShiftOp::Shl => { cpu.eflags = if cnt == 1 { let overflow = (dst >> 31) ^ (dst >> 30) & 1; (cpu.eflags & !(CF|OF)) | ((dst >> (32-cnt)) & 1) | (overflow << 11) } else { (cpu.eflags & !CF) | ((dst >> (32-cnt)) & 1) }; dst << cnt }
        ShiftOp::Shr => { cpu.eflags = (cpu.eflags & !CF) | ((dst >> (cnt-1)) & 1); dst >> cnt }
        ShiftOp::Sar => { cpu.eflags = (cpu.eflags & !CF) | ((dst >> (cnt-1)) & 1); ((dst as i32) >> cnt) as u32 }
        ShiftOp::Rol => dst.rotate_left(cnt),
        ShiftOp::Ror => dst.rotate_right(cnt),
    };
    set_szp(&mut cpu.eflags, result);
    match write_op(instr, 0, result, cpu, mem) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
}

fn exec_call(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let target = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
    let ret = cpu.eip;
    cpu.esp = cpu.esp.wrapping_sub(4);
    if let Err(e) = mem.write_u32(cpu.esp, ret) { return fault(e); }
    cpu.eip = target;
    if target >= TRAMPOLINE_BASE { StepResult::ApiTrap(target) } else { StepResult::Continue }
}

fn exec_ret(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory) -> StepResult {
    let ret = match mem.read_u32(cpu.esp) { Err(e) => return fault(e), Ok(v) => v };
    cpu.esp = cpu.esp.wrapping_add(4);
    // optional immediate (stdcall cleanup)
    if instr.op_count() > 0 {
        let imm = match read_op(instr, 0, cpu, mem) { Err(e) => return fault(e), Ok(v) => v };
        cpu.esp = cpu.esp.wrapping_add(imm);
    }
    cpu.eip = ret;
    if ret >= TRAMPOLINE_BASE { StepResult::ApiTrap(ret) } else { StepResult::Continue }
}
...
fn exec_setcc(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory, cond: bool) -> StepResult {
    match write_op(instr, 0, cond as u32, cpu, mem) { Err(e) => fault(e), Ok(()) => StepResult::Continue }
}

fn exec_stos(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory, sz: usize) -> StepResult {
    let count = if instr.has_rep_prefix() { cpu.ecx } else { 1 };
    let step = if get_df(cpu.eflags) { (0u32).wrapping_sub(sz as u32) } else { sz as u32 };
    for _ in 0..count {
        let r = match sz {
        ...
        if let Err(e) = r { return fault(e); }
        cpu.edi = cpu.edi.wrapping_add(step);
    }
    if instr.has_rep_prefix() { cpu.ecx = 0; }
    StepResult::Continue
}

fn exec_movs(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory, sz: usize) -> StepResult {
    let count = if instr.has_rep_prefix() { cpu.ecx } else { 1 };
    let step = if get_df(cpu.eflags) { (0u32).wrapping_sub(sz as u32) } else { sz as u32 };
    for _ in 0..count {
        let v = match sz {
        ...
        let v = match v { Err(e) => return fault(e), Ok(v) => v };
        let r = match sz {
...
fn exec_scas(instr: &Instruction, cpu: &mut X86Cpu, mem: &mut GuestMemory, sz: usize) -> StepResult {
    let repe = instr.has_rep_prefix();
    let repne = instr.has_repne_prefix();
    let step = if get_df(cpu.eflags) { (0u32).wrapping_sub(sz as u32) } else { sz as u32 };
    loop {
        if (repe || repne) && cpu.ecx == 0 { break; }
        let v = match sz {
        ...
        cpu.edi = cpu.edi.wrapping_add(step);
        if repe  { cpu.ecx -= 1; if !get_zf(cpu.eflags) { break; } }
        else if repne { cpu.ecx -= 1; if get_zf(cpu.eflags) { break; } }
        else { break; }
    }
    StepResult::Continue
}
