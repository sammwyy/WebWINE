...
pub fn register(r: &mut WinApiRegistry) {
    let fns: &[(&str, &str, super::HandlerFn)] = &[
        ...
        ("user32.dll", "GetDesktopWindow", |c| { c.ret_stdcall(0x0001_0000, 0); Handled::Ok }),
        ("user32.dll", "GetKeyboardLayout", |c| { c.ret_stdcall(0x0409_0409, 1); Handled::Ok }),
        ("user32.dll", "SetWinEventHook", |c| { c.ret_stdcall(1, 7); Handled::Ok }),
        ("user32.dll", "UnhookWinEvent", |c| { c.ret_stdcall(1, 1); Handled::Ok }),
        ("user32.dll", "SetActiveWindow", |c| { c.ret_stdcall(c.arg(0), 1); Handled::Ok }),
        ("user32.dll", "SetProcessDPIAware", |c| { c.ret_stdcall(1, 0); Handled::Ok }),
